...
import { checkProjectCommentT, checkProjectLangCodeT, checkProjectSubjectT, checkProjectOPT, type ProjectCommentT, type ProjectLangCodeT, type ProjectSubjectT, type ProjPerSub, type ProjectOPT } from './types';
...
async function fetchProjectsSubjects(): Promise<ProjectSubjectT[]> {
  ...
  if (!data.every(checkProjectSubjectT)) throw new Error("Invalid response from the Database...");
...
async function fetchCurrentProjectLangCode(projectID: string): Promise<ProjectLangCodeT> {
  const { data, error } = await supabase
    .from('projects')
    .select('lang,code')
    .eq('id', projectID)
  ...
  if (error) throw error;    
  if (!checkProjectLangCodeT(data)) throw new Error("Invalid response from the Database...");
...
async function fetchCurrentProjectComments(projectID: string): Promise<ProjectCommentT[]> {
  const { data, error } = await supabase
  ...
  if (error) throw error;
  if(!data.every(checkProjectCommentT)) throw new Error('Invalid response from the Database...');
...
export async function getCurrentProjectComments(projectID: string): Promise<{[key: string]: string}> {
...
async function fetchCurrentProjectOPImgs(projectID: string): Promise<ProjectOPT[]> {
  const { data, error } = await supabase
    ...
    if (error) throw error;
    if (!data.every(checkProjectOPT)) throw new Error("Invalid response from the Database...");
...
export async function getCurrentProjectOPImgs(projectID: string): Promise<{[key: string]: string}> {
