<script lang="ts">
  ...
  const { iconKey }: {iconKey: string} = $props();
</script>

<button class="header-btn w-8 h-8 bg-kwdr-fg rounded-lg flex justify-center items-center active:scale-95 transition-[scale] cursor-pointer">
  {#if iconKey==='info-icon'}
