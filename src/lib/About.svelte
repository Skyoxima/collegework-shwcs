...
<dialog
  class="w-9/10 md:w-1/2 h-max p-4 absolute top-1/2 left-1/2 -translate-1/2 bg-kwdr-fg rounded-2xl"
  bind:this={dialogRef}
  onclose={() => (modalSwitch.value = false)}
>
  <div class="relative mb-2 xl:mb-4">
    ...
    >
      <CloseIcon
        className="absolute size-4 xl:size-6 top-0 right-0 active:scale-70 transition-transform duration-100 ease-in cursor-pointer"
      />
    </button>
  </div>
